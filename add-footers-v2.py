...
for filepath in article_files:
    try:
        ...
        if '</body>' not in content:
            # File is substantial but missing closing tags - add them
            content += '\n' + footer_html + '\n</body>\n</html>'
            closing_added = True
        else:
        ...
        count += 1
        if count <= 20 or count % 50 == 0:  # Show first 20 and every 50th
            status = " (added closing tags)" if closing_added else ""
            print(f"✓ Added footer to: {filepath}{status}")
...
print(f"Incomplete files skipped: {incomplete}")
